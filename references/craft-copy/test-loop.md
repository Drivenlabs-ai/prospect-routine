...
1. **Résoudre** la campagne : `resolve --registry <racine Prospection/campaigns-registry.json> --slug <ce que dit l'utilisateur>` → `campaign_id` + `config_path`. Slug introuvable → demander lequel. Si l'appelant fournit déjà le `config_path` (campagne en cours de création, absente du registre), sauter cette étape.
...
4. **Assembler les deux jeux de fiches** : `prompts_before` = les fiches actuelles (toutes les clés) ; `prompts_after` = les mêmes, la clé éditée remplacée par la nouvelle version. En mode création (aucune fiche existante), passer `prompts_before` vide : le workflow saute le côté « avant » (aucun appel gaspillé) et ne génère que l'« après ». Écrire la nouvelle fiche dans un fichier local **temporaire** à ce stade — la fiche définitive n'est écrite qu'après validation (étape 7).
...
6. **Afficher côte à côte** : pour chaque prospect, montrer la version *avant* et la version *après* du (ou des) message(s), face à face, pour comparaison (en mode création, seul l'« après » existe). Laisser l'utilisateur juger sur l'axe qu'il visait. Ne pas parler de score ni de rubrique. Le test compare l'écriture brute : en production, un juge qualité repasse ensuite derrière le rédacteur (fond, pas longueur) et peut faire régénérer un message.
